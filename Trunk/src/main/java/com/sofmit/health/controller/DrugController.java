...
public class DrugController {
    ...
    public DrugDto save(@RequestBody DrugDto dto) {
        return drugConverter.toDto(drugService.save(dto)).get();
    }

    @PutMapping("{id}")
    @ResponseStatus(CREATED)
    @ApiOperation(value = "修改", notes = "update")
    public DrugDto update(@PathVariable("id") Long id, @RequestBody DrugDto dto) {
        return drugConverter.toDto(drugService.update(id, dto)).get();
    }
    ...
    public DrugDto findById(@PathVariable("id") Long id) {
        return drugConverter.toDto(drugService.findById(id)).orElseThrow(DataNotExistException::new);
    }

    @GetMapping(params = { "list" })
    public Iterable<DrugDto> list(@RequestParam Map<String, String> map) {
        return drugConverter.toDto(drugService.list(map));
    }
    ...
    public RangePage<DrugDto> list(
            @RequestParam Map<String, String> map,
            @PageableDefault Pageable pageable) {
        RangePage<Drug> result = drugService.list(map, pageable);
        return result.map(drugConverter::toDto).map(Optional::get);
    }
}
