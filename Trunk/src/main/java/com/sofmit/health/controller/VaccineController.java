...
public class VaccineController {
    ...
    public VaccineDto save(@RequestBody VaccineDto dto) {
        return vaccineConverter.toDto(vaccineService.save(dto)).get();
    }

    @PutMapping("{id}")
    @ResponseStatus(CREATED)
    @ApiOperation(value = "修改", notes = "update")
    public VaccineDto update(@PathVariable("id") Long id, @RequestBody VaccineDto dto) {
        return vaccineConverter.toDto(vaccineService.update(id, dto)).get();
    }
    ...
    public VaccineDto findById(@PathVariable("id") Long id) {
        return vaccineConverter.toDto(vaccineService.findById(id)).orElseThrow(DataNotExistException::new);
    }

    @GetMapping(params = { "list" })
    public Iterable<VaccineDto> list(@RequestParam Map<String, String> map) {
        return vaccineConverter.toDto(vaccineService.list(map));
    }
    ...
    public RangePage<VaccineDto> list(
            @RequestParam Map<String, String> map,
            @PageableDefault Pageable pageable) {
        RangePage<Vaccine> result = vaccineService.list(map, pageable);
        return result.map(vaccineConverter::toDto).map(Optional::get);
    }
}
